...
(define (make-log x)
  (cond ((number? x) (log x))
        (else (list 'log x))))
(define (make-expo x) (list 'expo x))
...
(define (expo? x)(and (pair? x)(eq? (car x) 'expo)))
...
(define (deriv expn var)
    (cond ((number? expn) 0)  
          ...
          ((expt? expn)(cond ((number? (multiplicand expn))(make-product (make-product (multiplicand exp)
          ...
          ((log? expn)(make-product (make-div 1 (ang expn))
                                    (deriv (ang expn) var)))
          ((expo? expn)(make-product (make-expo (ang expn))
                                    (deriv (ang expn) var)))
